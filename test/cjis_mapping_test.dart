...
void main() {
  group('CJIS Mapping and Risk Context Tests', () {
    test('All categories should have CJIS policy references', () {
      for (final category in GuidanceData.categories) {
        expect(category.cjisPolicyReferences, isNotNull,
            reason: 'Category ${category.title} should have CJIS policy references');
        expect(category.cjisPolicyReferences, isNotEmpty,
            reason: 'Category ${category.title} should have at least one CJIS policy reference');
      }
    });

    test('CJIS policy references should have section numbers and interpretations', () {
      for (final category in GuidanceData.categories) {
        for (final reference in category.cjisPolicyReferences) {
          expect(reference.sectionNumber, isNotEmpty,
              reason: 'Policy reference in ${category.title} should have section number');
          expect(reference.plainLanguageInterpretation, isNotEmpty,
              reason: 'Policy reference in ${category.title} should have plain language interpretation');
        }
      }
    });
    ...
    test('All categories should have risk context', () {
      for (final category in GuidanceData.categories) {
        ...
        expect(category.riskContext.riskPoints, isNotEmpty,
            reason: 'Risk context in ${category.title} should have risk points');
      }
    });
    ...
    test('All categories should have common failure patterns', () {
      for (final category in GuidanceData.categories) {
        expect(category.commonFailures, isNotNull,
            reason: 'Category ${category.title} should have common failures list');
        expect(category.commonFailures, isNotEmpty,
            reason: 'Category ${category.title} should have at least one common failure pattern');
      }
    });
      ...
      for (final category in GuidanceData.categories) {
        for (final failure in category.commonFailures) {
          expect(failure.pattern, isNotEmpty,
              reason: 'Failure pattern in ${category.title} should have pattern name');
          expect(failure.description, isNotEmpty,
              reason: 'Failure pattern in ${category.title} should have description');
        }
      }
    });
    ...
    test('All categories should have priority guidance', () {
      for (final category in GuidanceData.categories) {
        ...
        expect(category.priorityGuidance.higherRiskWhen, isNotEmpty,
            reason: 'Priority guidance in ${category.title} should have higher risk condition');
        expect(category.priorityGuidance.lowerRiskWhen, isNotEmpty,
            reason: 'Priority guidance in ${category.title} should have lower risk condition');
      }
    });
    ...
    test('Access Control should have specific CJIS references', () {
      ...
      final hasSection52 = category.cjisPolicyReferences.any(
        (ref) => ref.sectionNumber.contains('5.2')
      );
    ...
    test('Authentication and MFA should have specific CJIS references', () {
      ...
      final hasSection56 = category!.cjisPolicyReferences.any(
        (ref) => ref.sectionNumber.contains('5.6')
      );
    ...
    test('Data Storage should have specific CJIS references', () {
      ...
      final hasSection510 = category!.cjisPolicyReferences.any(
        (ref) => ref.sectionNumber.contains('5.10')
      );
    ...
    test('Risk context titles should be appropriate', () {
      for (final category in GuidanceData.categories) {
        ...
        expect(hasAppropriateTitle, isTrue,
            reason: 'Risk context title in ${category.title} should mention risk, why, or matter');
      }
    });
    ...
    test('Plain language interpretations should not be verbatim policy quotes', () {
      ...
      for (final category in GuidanceData.categories) {
        for (final reference in category.cjisPolicyReferences) {
          final interpretation = reference.plainLanguageInterpretation.toLowerCase();
          
          // Plain language should avoid formal policy language
          final startsWithPolicyLanguage = policyLanguagePatterns.any(
            (pattern) => interpretation.startsWith(pattern)
          );
          
          expect(startsWithPolicyLanguage, isFalse,
              reason: 'Policy interpretation in ${category.title} should use plain language, not policy quotes');
        }
      }
    });
    ...
    test('Each category should have at least 3 risk points', () {
      for (final category in GuidanceData.categories) {
        expect(category.riskContext.riskPoints.length, greaterThanOrEqualTo(3),
            reason: 'Category ${category.title} should have at least 3 risk points');
      }
    });
    ...
    test('Each category should have at least 3 common failure patterns', () {
      for (final category in GuidanceData.categories) {
        expect(category.commonFailures.length, greaterThanOrEqualTo(3),
            reason: 'Category ${category.title} should have at least 3 common failure patterns');
      }
    });
  });
}
